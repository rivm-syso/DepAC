...
contains
    ...
    subroutine rc_gstom(comp, lu_conf, meteo, dp_conf, dp_out, err)
        ...
        select case (comp%index)
            case (COMP_NO)
                ! NO: stomatal uptake neglected (no conductance)
                dp_out%gstom = 0.0

            case (COMP_NO2, COMP_O3, COMP_SO2, COMP_NH3)
                ! For these components, compute stomatal conductance if vegetation is present and
                ! global radiation is positive
                if (dp_conf%has_vegetation) then
                    if (meteo%glrad > 0.0) then
                        call rc_get_vpd(meteo, vpd)
                        ! Scale by component diffusion coefficient
                        dp_out%gstom = rc_gstom_emb(lu_conf, meteo, vpd, dp_conf) * comp%diffc / &
                             dp_conf%dO3
                    else
                        dp_out%gstom = 0.0
                    end if
            ...
            case default
                call set_error(err, ERR_INPUT, 'Component '//trim(comp%name)//&
                    ' not supported in rc_gstom')
                call log_error('Component '//trim(comp%name)//' not supported in rc_gstom')
                return
        end select
    ...
    !------------------------------------------------------------------------------
    ! Subroutine: rc_get_vpd
    ! Purpose   : Calculate vapour pressure deficit (VPD) from meteorological data
    ! Arguments :
    !   meteo - meteorology type
    !   vpd   - output VPD (kPa)
    ! Notes     : Uses Monteith (1973) fit for saturation vapour pressure
    !------------------------------------------------------------------------------
    subroutine rc_get_vpd(meteo, vpd)
    ...
    !------------------------------------------------------------------------------
    subroutine par_dir_diff(meteo, par_dir, par_diff)
        type(depac_meteorology), intent(in) :: meteo ! meteorology
    ...
    !------------------------------------------------------------------------------
    function rc_gstom_emb(lu_conf, meteo, vpd, dp_conf) result(gstom)
        ...
        if (dp_conf%has_vegetation) THEN
            ...
            if (meteo%glrad > 200 .and. dp_conf%lai > 2.5) then
                PARshade = PARdiff*exp(-0.5*dp_conf%lai**0.8) + &
                    0.07*PARdir*(1.1-0.1*dp_conf%lai)*exp(-meteo%sinphi)
            else
            ...
            if (meteo%glrad > 200 .and. dp_conf%lai > 2.5) then
                PARsun = PARdir**0.8*0.5/meteo%sinphi + PARshade
            else
                PARsun = PARdir*0.5/meteo%sinphi + PARshade
            end if
            ! Leaf area index for sunlit and shaded leaves
            ! only works for sinphi > 0, but this is already checked and adjusted above
            LAIsun = 2*meteo%sinphi*(1-exp(-0.5*dp_conf%lai/meteo%sinphi ))
